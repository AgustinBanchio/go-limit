...
func TestRollingWindow_Allowed(t *testing.T) {
	t.Parallel()
...
func TestRollingWindow_Reserve_BlocksAfterCapacityReached(t *testing.T) {
	t.Parallel()

	// 3 requests per second
	limiter := limit.NewRollingWindow(3, 1*time.Second)
...
func TestRollingWindow_Reserve_CancelFreesCapacity(t *testing.T) {
	t.Parallel()

	// 2 requests per second
	limiter := limit.NewRollingWindow(2, 1*time.Second)
